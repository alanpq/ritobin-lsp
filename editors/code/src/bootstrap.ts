...
async function getServer(
  ...
  const explicitPath =
    process.env["__RB_LSP_SERVER_DEBUG"] ?? config.serverPath;
  if (explicitPath) {
...
async function hasToolchainFileWithRaDeclared(
  uri: vscode.Uri,
): Promise<boolean> {
  try {
    ...
    return (
      toolchainFileContents.match(/components\s*=\s*\[.*"ritobin-lsp".*\]/g)
        ?.length === 1
    );
  } catch (_) {
    return false;
