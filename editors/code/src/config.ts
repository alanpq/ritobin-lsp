...
export class Config {
  readonly extensionId = "alanpq.ritobin-lsp-vs";

  configureLang: vscode.Disposable | undefined;
  workspaceState: vscode.Memento;

  private readonly rootSection = "ritobin-lsp";
  ...
  /// Returns the ritobin-lsp-specific workspace configuration, incl. any
  ...
  async addExtensionConfiguration(
    ...
    await this.workspaceState.update(
      this.extensionConfigurationStateKey,
      extCfgs,
    );
  ...
  private async onDidChangeConfiguration(
    ...
    if (this.restartServerOnConfigChange) {
      await vscode.commands.executeCommand("ritobin-lsp.restartServer");
      return;
    }
    ...
    if (userResponse) {
      const command = "ritobin-lsp.restartServer";
  ...
  // Returns the raw configuration for ritobin-lsp as returned by vscode. This
  ...
  public get cfg(): ConfigurationTree {
    ...
    for (const [extensionId, items] of Object.entries(
      ...
      for (const [k, v] of Object.entries(items)) {
        ...
        log.trace(
          `Extension ${extensionId} overrides configuration ${k} to `,
          v,
        );
        set(finalConfig, k, v);
      }
  ...
  async toggleCheckOnSave() {
    const config = this.rawCfg.inspect<boolean>("checkOnSave") ?? {
      key: "checkOnSave",
    };
  ...
  runnablesExtraEnv(label: string): Env {
    ...
    let extraEnv =
      this.get<
        | RunnableEnvCfgItem[]
        | { [key: string]: { toString(): string } | null }
        | null
      >("runnables.extraEnv") ?? {};
...
export function prepareVSCodeConfig(
  resp: ConfigurationValue,
): ConfigurationValue {
  if (Is.string(resp)) {
